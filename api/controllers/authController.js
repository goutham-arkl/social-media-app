...
exports.register= async (req, res) => {
    try {
      ...
      if (req.body.username && req.body.email && req.body.password) {
        ...
        }).catch((err)=>{console.log(err); res.status(400).json({"msg":"user already exist",err})});
...
exports.userLogin=async (req, res) => {
    try {
      if (req.body.email && req.body.password) {
        ...
        console.log('agfukgfigdsftlidhfsgodhglfd');
        console.log(user);
  ...
  exports.sentotpHandler=async(req, res) =>{
    ...
    if(!user||user==null){
      ...
      res.status(403).json("please register before login")
        ...
        .catch((err) => {
          console.log('dsgfjshgkjghkdsfghdhgkghsdjghkdfj');
          console.log(err);
